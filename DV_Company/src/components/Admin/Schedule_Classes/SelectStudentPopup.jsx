...
import { selectStudentDetails } from "../../../constants/selectStudentDetails";
import { IMAGES } from "../../../assets";

export default function SelectStudentPopup({
  toggleFunc,
  setSelectedStudents,
  selectedStudents = "",
}) {
  const [students, setStudents] = useState(selectStudentDetails);
              ...
              {filteredStudents.length > 0 ? (
                filteredStudents.map((student, index) => (
                  <SelectStudentCard
                    ...
                    selected={
                      selectedStudents && selectedStudents.key === index
                    }
                    onClick={() =>
                      toggleSelectStudent({ ...student, key: index })
                    }
                  />
                ))
              ) : (
