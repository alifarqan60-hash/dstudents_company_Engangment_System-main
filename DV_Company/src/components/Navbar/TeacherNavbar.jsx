...
export default function StudentNavbar({ name, img }) {
  ...
  return (
    <div className="flex flex-row w-full h-[80px] rounded-tl-3xl md:pl-10 items-center font-sans font-medium text-base">
      ...
      {/* Notification Icon  */}
      <div className="flex-1 ml-5  border-l border-l-gray-200 pl-4 pr-3 md:pr-2 lg:pr-1 xl:pr-0">
        {isPopupVisible && <NotificationPopup toggleFunc={togglePopup} />}
        <TbBell size={22} />
        <GoDotFill
          className="text-customLightRed cursor-pointer absolute top-9 ml-[10px] "
          size={13}
          onClick={togglePopup}
        />
      </div>
