...
import { deleteQuiz } from "../../../api/Quizzes/deleteQuiz";
// import { deleteUser } from "../../../api/Users/deleteUser";

export default function UserDeletedPopup({ user, toggleFunc }) {
  const [loader, setLoader] = useState(false);

  const handleDeleteClick = async () => {
    setLoader(true);
    console.log("delete function called: ", user.id);
    try {
      await deleteQuiz(user.id);
      console.log("quiz deleted, CLG in delete function");
      ...
      <div className="fixed inset-0 flex items-center justify-center h-screen w-auto z-10">
        <div className="w-[320px] bg-customPopupBgColor p-4 rounded-3xl font-sans space-y-3 border border-gray-300">
          ...
          <div className="flex-[2] font-normal text-sm text-customPopupTextColor mb-2">
            ...
            <span className="name font-semibold text-customDarkBlue"></span>{" "}
            this quiz from the system?
          </div>
