...
export default function SelectTeacherPopup({
  ...
  const toggleSelectTeacher = (user) => {
    setSelectedTeacher(user); // Set the selected teacher
  };
        ...
        <div className="w-[400px] bg-customPopupBgColor p-4 pt-2 rounded-3xl font-sans space-y-3 border border-gray-300">
          ...
          <div className="flex justify-between">
            ...
            <div className="flex-[3] flex justify-between">
              <p className="font-medium ml-5 mt-3 text-xl">Select Owner</p>
              <div
                onClick={toggleFunc}
              ...
              <div className="w-full h-[220px] overflow-y-auto register-scrollbar2 ">
                {filteredUsers.length > 0 ? (
                  filteredUsers.map((user, index) => (
                    <SelectTeacherCard
                      key={user._id || index}
                      img={user.photo || user.imgUrl}
                      name={user.username}
                      email={user.email}
                      bgColor={giveBgColor(index)}
                      selected={
                        selectedTeacher && selectedTeacher._id === user._id
                      }
                      onClick={() =>
                        toggleSelectTeacher(user)
                      }
                    />
                  ))
                ) : (
