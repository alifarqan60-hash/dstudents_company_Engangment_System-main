...
export default function Navbar({ heading }) {
  ...
  // Pull the admin user from Redux store
  const adminUser = useSelector((state) => state.auth?.user);
  const name = adminUser?.username || "Admin";
  const role = adminUser?.isAdmin ? "Welcome M.Danial" : "Admin";
  const avatar = adminUser?.imgUrl || adminUser?.photo || IMAGES.admin_profile;
        ...
        >
          ...
          <div className="flex justify-center md:justify-normal md:flex-none pl-3 mdLg:pl-4 lg:pl-5">
            <img
              src={avatar}
              alt="Profile Pic"
              onError={(e) => { e.target.src = IMAGES.admin_profile; }}
              className={`h-[38px] w-[38px] object-cover border-2 border-gray-200 ${avatar === IMAGES.admin_profile ? "logo-diamond logo-contrast-light" : "rounded-full"}`}
            />
          </div>

          {/* Text  */}
          <div className="ml-20 flex-1 md:flex-auto md:mr-0 md:ml-4 hidden md:block">
            <p className="font-bold text-customDarkBlue text-sm font-inter truncate max-w-[120px]">
              {name}
            </p>
            <p className="text-customRoleColor text-xs font-inter">{role}</p>
          </div>
        </div>
      </div>
    </div>
